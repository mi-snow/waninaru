...
class DirectMessagesController extends AppController {
	...
	public function view($id = null) {
		...
		$options = array (
				'conditions' => array (
						'DirectMessage.' . $this->DirectMessage->primaryKey => $id 
				),
				'recursive' => 1 
		);
		...
		if ($data ['DirectMessage'] ['send_mode'] == 2) {
			$joiner_id = $this->DirectMessage->find ( 'all', array (
					'fields' => array (
							'joiner_id' 
					),
					'conditions' => array (
							'DirectMessage.created' => $data ['DirectMessage'] ['created'],
							'DirectMessage.project_id' => $data ['DirectMessage'] ['project_id'],
							'DirectMessage.producer_id' => $data ['DirectMessage'] ['producer_id'] 
					) 
			) );
			$joinerAll = null;
			foreach ( $joiner_id as $joiner ) {
				$user_id = $this->Joiner->find ( 'first', array (
						...
						'conditions' => array (
								'Joiner.id' => $joiner ['DirectMessage'] ['joiner_id'] 
						),
						'recursive' => - 1 
				) );
				$student_number = $this->User->find ( 'first', array (
						...
						'conditions' => array (
								'User.id' => $user_id ['Joiner'] ['user_id'] 
						),
						'recursive' => - 1 
				) );
				
				// 学生の名前を出力
				$student_name = $this->User->find ( 'first', array (
						'fields' => array (
								'real_name' 
						),
						'conditions' => array (
	...
	public function joiner_add($id = null, $id2 = null) { // joiner(参加者)がproducer(企画者)に送る
		...
		$producerList = $this->ProducersProject->find ( 'all', array (
				'conditions' => array (
						'ProducersProject.project_id' => $id,
						'ProducersProject.producer_id' => $producer ['Producer'] ['id'] 
				) 
		) );
		...
		$producer_name = $this->Project->find ( 'first', array (
				'conditions' => array (
						'Project.id' => $id 
				) 
		) );
		$producer_id = $this->ProducersProject->find ( 'first', array (
				'conditions' => array (
		...
		$joiner_id = $this->Joiner->find ( 'first', array (
				"fields" => 'Joiner.id',
				"conditions" => array (
						"Joiner.user_id" => $userSession ['id'] 
				) 
		) );
		$delete_flag = $this->Project->find ( 'first', array (
				"fields" => 'Project.delete_flag',
				"conditions" => array (
						"Project.id" => $id 
				) 
		) );
		$safety_check = $this->JoinersProject->find ( 'count', array (
				'fields' => Array (
						'JoinersProject.joiner_id' 
				),
				'conditions' => array (
						'JoinersProject.joiner_id' => $joiner_id ['Joiner'] ['id'],
		...
		if ($this->request->is ( 'post' )) {
			...
			if ($this->DirectMessage->save ( $data )) {
				...
				// メール送信　宛先:企画者
				$user_id= $this->Producer->find( 'list', array (
					"fields" => 'user_id',
					"conditions" => array (
							"Producer.id" => $data ['producer_id'] 
					) 
				));
				$student_number=$this->User->find('first',array(
...
/*				if ((260600 <= $student_number) && ($student_number <= 260999)) { // テスト用
	...
	public function producer_add($id = null, $id2 = null) { // producer(企画者)がjoiner(参加者)に送る
		...
		$safety_check = $this->ProducersProject->find ( 'count', array (
				'fields' => Array (
						'ProducersProject.project_id' 
				),
				'conditions' => array (
						'ProducersProject.project_id' => $project ['Project'] ['id'],
		...
		if ($this->request->is ( 'post' )) {
			...
			$student_number=$joiner_id;
			$JoinerAll = $joiner_id;
			$joiner = $this->User->find ( 'first', array (
					'fields' => 'id',
					'conditions' => array (
							'User.student_number' => $joiner_id 
					) 
			) );
			$joiner = $this->Joiner->find ( 'first', array (
					'fields' => 'id',
					'conditions' => array (
							'Joiner.user_id' => $joiner [User] [id] 
					) 
			) );
			$data ['joiner_id'] = $joiner ['Joiner'] ['id'];
			$data ['send_mode'] = $id2;
			if ($this->DirectMessage->save ( $data )) {
				$message_id = $this->DirectMessage->find ( 'first', array (
